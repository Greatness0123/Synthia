...
3. Create a new code cell at the top of the notebook and paste the setup dependencies:
   ```python
   !pip install -q transformers accelerate bitsandbytes fastapi uvicorn pydantic Pillow torch schedule qwen-vl-utils
   ```
...
2. Search for **Qwen2.5-VL-7B-Instruct** (or **Qwen2.5-VL-3B-Instruct**) in the *Models* tab.
...
5. *(Optional)* If your path is slightly different, find it in the sidebar file explorer under **input**, click the copy icon next to the folder, and update the `MODEL_PATH` variable in `kaggle_inference.py`. (The script automatically attempts to find it at standard Kaggle paths!).
...
5. Click **Test** to verify connection, then click **Apply & Save**!
