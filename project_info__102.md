...
**R5 — Root-motion forward vector is assumed, not measured.** The converter maps Mixamo `posZ` to `dz = ΔposZ × 0.01 × (−1)`. The real clip's forward axis (Mixamo +Z forward) is hard-coded; if the source clip turns or is authored in a different convention, the agent walks backward or sideslips.
...
**What:** Give the root capsule real mass + inertia (≥ 55 kg, realistic diaginertia ~2.5/1.2/2.5 for a 1.8 m, 0.2-radius capsule), keep `contype="0"` if collision-off is needed, and re-tune PD gains so the trunk hangs *through* the root rather than *off* an inertialess point. Also give the feet proper foot geometry with a friction mat and toe contact.
...
*This report was saved to `project_info__101.md` in the project root.*

---

## Important note on your request #3

You asked for **scripts** that simulate small movements. I'm in **Explore Mode** — a codebase investigation mode. I can analyze code and produce documentation, but I can't write scripts or modify files here. The primitives I've documented above (parameter tables + sequence patterns in §3) give you everything needed to author those scripts in Act Mode: each row is directly dispatchable via the existing `synthia:action` event contract, and the walk-cycle micro-sequence is a complete ready-to-paste 5-frame timeline. To have the scripts actually written into the repo (e.g., a new `scripts/smallMotionPrimitives.ts` + browser console tool), switch to **Act Mode** using the mode selector at the bottom of the chat — my exploration findings will carry over as context.