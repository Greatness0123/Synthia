...
The `tool_tip_site` is already referenced in `ik_demo.py` via `mj_name2id(..., mjOBJ_SITE, "tool_tip_site")` — but the generated MJCF from `MJCFHumanoidTemplate.ts` does NOT define a `<site>` element. Adding one to the dynamically-generated MJCF would be trivial (a few lines in the XML template) and is the prerequisite for porting the IK logic.